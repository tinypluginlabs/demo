...
import { phpVar as c, phpVars as p } from "@php-wasm/util";
import { hydrateUpdateFileOps as m, normalizeFilesystemOperations as y } from "@php-wasm/fs-journal";
const h = `<?php
...
async function f(e) {
  await e.fileExists("/wordpress/wp-content/mu-plugins") || await e.mkdir("/wordpress/wp-content/mu-plugins"), await e.writeFile(
...
async function g(e, n, t = {}) {
  ...
  return d(s, "Initialization failed."), (await e.run({
    code: `<?php
        require '/wordpress/wp-load.php';
		$data = $GLOBALS['@pdo']
...
async function E(e, n) {
  let t = null;
  e.addEventListener("request.end", () => {
    t = null;
  }), e.onMessage(async (s) => {
    const r = JSON.parse(s);
    if ((r == null ? void 0 : r.type) === "sql") {
      if (r.subtype === "transaction") {
        const o = r;
        if (!o.success)
          return;
        switch (o.command) {
          case "START TRANSACTION":
            ...
            break;
          case "COMMIT":
            t != null && t.length && t.forEach(n), t = null;
            break;
          case "ROLLBACK":
            t = null;
            break;
      ...
      if (r.subtype === "replay-query" || r.subtype === "reconstruct-insert") {
        const o = r;
        t ? t.push(o) : n(o);
      }
    }
...
async function $(e, n) {
  const t = p({ journal: n }), s = await e.run({
    code: `<?php
...
function d(e, n) {
  if (e.text.trim() || e.errors.trim())
    throw u.error({
      text: e.text,
      errors: e.errors
    }), new Error(`${n}. See the console for more details.`);
}
async function q(e, n) {
  await e.journalFSEvents(
    "/wordpress/wp-content",
    async (t) => {
      t.path.endsWith("/.ht.sqlite.php") || t.path.endsWith("/.ht.sqlite.php-journal") || t.path.endsWith("/.ht.sqlite") || t.path.endsWith("/.ht.sqlite-journal") || n(t);
    }
  );
}
class v {
  ...
  onChangesReceived(n) {
    window.addEventListener("message", (t) => {
      t.data.type === "playground-change" && n(t.data.envelope);
    });
  }
}
class N {
...
const O = (e) => ({
  beforeSend: (n) => ({
    ...n,
    sql: n.sql.map((t) => (t.subtype === "reconstruct-insert" && e(t.table_name, t.last_insert_id), t))
  }),
  afterReceive: (n) => n
});
function C(e) {
  return {
    beforeSend: (n) => ((n.sql.length > 0 || n.fs.length > 0) && u.log(`[${e}] Sending changes`, n), n),
    afterReceive: (n) => (u.log(`[${e}] Received changes`, n), n)
  };
}
const w = (e, n = "https://playground.wordpress.net") => ({
  beforeSend: _(e, n),
  afterReceive: _(n, e)
});
function _(e, n) {
  ...
  return (r) => ({
    ...
    sql: r.sql.map((o) => {
      if (o.subtype === "replay-query")
        return {
          ...o,
          query: o.query.replace(e, n)
        };
      if (o.subtype === "reconstruct-insert") {
        const i = { ...o.row };
        for (const a in i)
          typeof i[a] == "string" && (i[a] = i[a].replace(
            t,
            s
          ));
        return {
...
const S = () => ({
  ...
  var s;
  if (e.query_type === "SELECT")
    return !1;
  const n = e.query_type, t = (s = e.table_name) == null ? void 0 : s.toLowerCase();
  if (e.subtype === "replay-query") {
    const r = e.query.trim();
    if (n === "UPDATE" && t === "wp_options" && r.endsWith("`option_name` = 'cron'"))
      return !1;
  }
  if (e.subtype === "reconstruct-insert") {
    if (t === "wp_options") {
      const r = e.row.option_name + "";
      if (r.startsWith("_transient_") || r.startsWith("_site_transient_"))
        return !1;
    }
    if (t === "wp_usermeta" && e.row.meta_key === "session_tokens")
      return !1;
  }
  return !0;
}, R = (e) => ({
...
async function Q(e, { autoincrementOffset: n, transport: t, middlewares: s = [] }) {
  s = [
    S(),
    w(await e.absoluteUrl),
    R(e),
    ...s
  ], await f(e), await g(e, n), t.onChangesReceived(async (a) => {
    for (const l of s)
      a = await l.afterReceive(a);
    await e.replayFSJournal(a.fs), await $(e, a.sql);
  });
  let r = { fs: [], sql: [] };
  E(e, (a) => {
    r.sql.push(a);
  ...
  const o = async () => {
    ...
    r = { fs: [], sql: [] };
    for (const l of s)
      a = await l.beforeSend(a);
    !a.sql.length && !a.fs.length || t.sendChanges(a);
  }, i = async (a, l) => {
    await a(), setTimeout(i, l, a, l);
  };
  i(o, 3e3);
}
export {
  N as NoopTransport,
  v as ParentWindowTransport,
  f as installSqlSyncMuPlugin,
  q as journalFSOperations,
  E as journalSQLQueries,
  C as loggerMiddleware,
  w as marshallSiteURLMiddleware,
  g as overrideAutoincrementSequences,
  $ as replaySQLJournal,
  Q as setupPlaygroundSync,
  O as trackAutoincrementMiddleware
};
