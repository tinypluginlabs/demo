"use strict";Object.defineProperty(exports,Symbol.toStringTag,{value:"Module"});const u=require("@php-wasm/logger"),c=require("@php-wasm/util"),_=require("@php-wasm/fs-journal"),q=`<?php
...
playground_sync_start();`;async function p(e){await e.fileExists("/wordpress/wp-content/mu-plugins")||await e.mkdir("/wordpress/wp-content/mu-plugins"),await e.writeFile("/wordpress/wp-content/mu-plugins/sync-mu-plugin.php",q)}async function m(e,n,t={}){const s=await e.run({code:`<?php
        require '/wordpress/wp-load.php';
        playground_sync_override_autoincrement_algorithm(
			${c.phpVar(n)},
			${c.phpVar(t)}
		);
	    `});return f(s,"Initialization failed."),(await e.run({code:`<?php
        require '/wordpress/wp-load.php';
		$data = $GLOBALS['@pdo']
			->query('SELECT * FROM playground_sequence')
			->fetchAll(PDO::FETCH_KEY_PAIR);
		echo json_encode($data);
		`})).json}async function y(e,n){let t=null;e.addEventListener("request.end",()=>{t=null}),e.onMessage(async s=>{const r=JSON.parse(s);if((r==null?void 0:r.type)==="sql"){if(r.subtype==="transaction"){const o=r;if(!o.success)return;switch(o.command){case"START TRANSACTION":t=[];break;case"COMMIT":t!=null&&t.length&&t.forEach(n),t=null;break;case"ROLLBACK":t=null;break}return}if(r.subtype==="replay-query"||r.subtype==="reconstruct-insert"){const o=r;t?t.push(o):n(o)}}})}async function h(e,n){const t=c.phpVars({journal:n}),s=await e.run({code:`<?php
	...
	`});f(s,"Replay error.")}function f(e,n){if(e.text.trim()||e.errors.trim())throw u.logger.error({text:e.text,errors:e.errors}),new Error(`${n}. See the console for more details.`)}async function g(e,n){await e.journalFSEvents("/wordpress/wp-content",async t=>{t.path.endsWith("/.ht.sqlite.php")||t.path.endsWith("/.ht.sqlite.php-journal")||t.path.endsWith("/.ht.sqlite")||t.path.endsWith("/.ht.sqlite-journal")||n(t)})}class ${sendChanges(n){window.top.postMessage({type:"playground-change",envelope:n},"*")}onChangesReceived(n){window.addEventListener("message",t=>{t.data.type==="playground-change"&&n(t.data.envelope)})}}class w{sendChanges(){}onChangesReceived(n){this.injectChanges=n}injectChanges(n){}}const S=e=>({beforeSend:n=>({...n,sql:n.sql.map(t=>(t.subtype==="reconstruct-insert"&&e(t.table_name,t.last_insert_id),t))}),afterReceive:n=>n});function T(e){return{beforeSend:n=>((n.sql.length>0||n.fs.length>0)&&u.logger.log(`[${e}] Sending changes`,n),n),afterReceive:n=>(u.logger.log(`[${e}] Received changes`,n),n)}}const E=(e,n="https://playground.wordpress.net")=>({beforeSend:d(e,n),afterReceive:d(n,e)});function d(e,n){e=e.replace(/\/$/,""),n=n.replace(/\/$/,"");const t=new RegExp(`(^|[^0-9a-zA-Z])(?:${b(e)})($|[^0-9a-zA-Z])`,"g"),s=`$1${n}$2`;return r=>({...r,sql:r.sql.map(o=>{if(o.subtype==="replay-query")return{...o,query:o.query.replace(e,n)};if(o.subtype==="reconstruct-insert"){const i={...o.row};for(const a in i)typeof i[a]=="string"&&(i[a]=i[a].replace(t,s));return{...o,row:i}}return o})})}function b(e){return e.replace(/[/\-\\^$*+?.()|[\]{}]/g,"\\$&")}const L=()=>({beforeSend:e=>({...e,sql:e.sql.filter(R)}),afterReceive:e=>e}),R=e=>{var s;if(e.query_type==="SELECT")return!1;const n=e.query_type,t=(s=e.table_name)==null?void 0:s.toLowerCase();if(e.subtype==="replay-query"){const r=e.query.trim();if(n==="UPDATE"&&t==="wp_options"&&r.endsWith("`option_name` = 'cron'"))return!1}if(e.subtype==="reconstruct-insert"){if(t==="wp_options"){const r=e.row.option_name+"";if(r.startsWith("_transient_")||r.startsWith("_site_transient_"))return!1}if(t==="wp_usermeta"&&e.row.meta_key==="session_tokens")return!1}return!0},I=e=>({beforeSend:async n=>({...n,fs:await _.hydrateUpdateFileOps(e,_.normalizeFilesystemOperations(n.fs))}),afterReceive:n=>n});async function A(e,{autoincrementOffset:n,transport:t,middlewares:s=[]}){s=[L(),E(await e.absoluteUrl),I(e),...s],await p(e),await m(e,n),t.onChangesReceived(async a=>{for(const l of s)a=await l.afterReceive(a);await e.replayFSJournal(a.fs),await h(e,a.sql)});let r={fs:[],sql:[]};y(e,a=>{r.sql.push(a)}),g(e,a=>{r.fs.push(a)});const o=async()=>{let a=r;r={fs:[],sql:[]};for(const l of s)a=await l.beforeSend(a);!a.sql.length&&!a.fs.length||t.sendChanges(a)},i=async(a,l)=>{await a(),setTimeout(i,l,a,l)};i(o,3e3)}exports.NoopTransport=w;exports.ParentWindowTransport=$;exports.installSqlSyncMuPlugin=p;exports.journalFSOperations=g;exports.journalSQLQueries=y;exports.loggerMiddleware=T;exports.marshallSiteURLMiddleware=E;exports.overrideAutoincrementSequences=m;exports.replaySQLJournal=h;exports.setupPlaygroundSync=A;exports.trackAutoincrementMiddleware=S;
